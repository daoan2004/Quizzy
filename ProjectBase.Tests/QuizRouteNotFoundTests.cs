...
public sealed class QuizRouteNotFoundTests : IClassFixture<QuizzyWebApplicationFactory>
{
    private readonly HttpClient _client;

    public QuizRouteNotFoundTests(QuizzyWebApplicationFactory factory)
    {
    ...
    public async Task Submit_answer_returns_not_found_before_database_update()
    {
        ...
        using var response = await _client.PostAsync("/api/QuizApi/submitAnswer", content);
        Assert.Equal(HttpStatusCode.NotFound, response.StatusCode);
    }
}
